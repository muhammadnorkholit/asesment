...
@section('main')
   ...
   <form action="/admin/mapel/{{$data->id}}" method="POST">
    @method('PUT')
    @csrf
  <div class="mb-3">
    <label for="mapel" class="form-label text-capitalize">nama mapel</label>
    <input type="text" class="form-control @error('mapel') is-invalid @enderror" name="mapel" value="{{$data->nama_mapel}}" id="mapel" aria-describedby="emailHelp">
    @error('mapel')
